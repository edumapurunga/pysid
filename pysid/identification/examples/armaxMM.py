"""
    In this example we use the SysID library to estimate a MIMO armax model
"""
#Import Libraries
...
for i in range(2, N):
    y1[i] = -dot(A1o[1:3] , y1[i-2:i][::-1]) - dot(A12o[1:3],y2[i-2:i][::-1]) + dot(B11o[1:3], u[i-2:i, 0][::-1]) + dot(B12o[1:3], u[i-2:i, 1][::-1])
    y2[i] = -dot(A21o[1:3], y1[i-2:i][::-1]) - dot(A2o[1:3], y2[i-2:i][::-1]) + dot(B21o[1:3], u[i-2:i, 0][::-1]) + dot(B22o[1:3], u[i-2:i, 1][::-1])
